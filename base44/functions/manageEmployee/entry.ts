...
Deno.serve(async (req) => {
  try {
    ...
    // Igual que 'resendWelcome' pero para todos los empleados activos con
    // email + usuario a la vez — cada uno recibe una contraseña nueva propia.
    if (action === 'resendWelcomeAll') {
      const employees = await base44.asServiceRole.entities.Employee.list('-created_date', 200);
      const results = [];

      for (const emp of employees) {
        if (!emp.is_active) { results.push({ id: emp.id, name: emp.full_name, error: 'Empleado inactivo — omitido' }); continue; }
        if (!emp.email) { results.push({ id: emp.id, name: emp.full_name, error: 'Sin email registrado — omitido' }); continue; }
        if (!emp.user) { results.push({ id: emp.id, name: emp.full_name, error: 'Sin usuario asignado — omitido' }); continue; }
        ...
        try {
          ...
          results.push({ id: emp.id, name: emp.full_name, sentTo: emp.email, sent: true });
        } catch (emailError) {
          results.push({ id: emp.id, name: emp.full_name, error: emailError.message });
        }
      }
